<template>
  <section class="tool-page">
    ...
    <main class="page-content">
      <ImageResizer @resize="handleResize" />
    </main>
      ...
      <div class="modal-card" @click.stop>
        ...
        <div class="modal-body">
          <div class="help-section">
            <h4>基本操作</h4>
            <ul>
              <li>点击"添加图片"或拖拽图片到指定区域</li>
              <li>支持批量选择多张图片同时处理</li>
              <li>设置目标尺寸（宽度和高度）</li>
              <li>点击"批量调整"开始处理</li>
            </ul>
          </div>
          <div class="help-section">
            <h4>尺寸设置</h4>
            <ul>
              <li>可以只设置宽度或高度，另一边自动按比例计算</li>
              <li>同时设置宽高会按指定尺寸调整</li>
              <li>支持预设常用尺寸快速选择</li>
            </ul>
          </div>
          <div class="help-section">
            <h4>批量处理</h4>
            <ul>
              <li>所有图片将按相同的设置进行调整</li>
              <li>处理完成后可批量下载所有图片</li>
              <li>支持选择输出文件夹保存</li>
            </ul>
          </div>
        </div>
    ...
    <!-- 处理成功提示 -->
    <div v-if="resizeSuccess" class="toast-success">
...
<script setup>
import { ref } from 'vue'
import ImageResizer from '@/components/image/ImageResizer.vue'

const showHelp = ref(false)
const resizeSuccess = ref(false)

const handleResize = (result) => {
  resizeSuccess.value = true
  setTimeout(() => {
    resizeSuccess.value = false
  }, 3000)

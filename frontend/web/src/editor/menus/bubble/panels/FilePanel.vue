...
const getFileBlockOptions = () => {
  ...
  return (ext?.options || {}) as {
    resolveDownloadUrl?: (attrs: NodeAttrs) => string
    resolveShareUrl?: (attrs: NodeAttrs) => string
    onDownload?: (attrs: NodeAttrs, context: { editor: Editor }) => Promise<void>
    downloadStrategy?: 'auto' | 'blob' | 'a'
...
const downloadByBlob = async (url: string, fileName: string) => {
